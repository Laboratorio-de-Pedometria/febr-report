...
# Criar figura
dev.off()
png("res/fig/event-distribution-in-space-and-time.png",
...
for (i in 1:6) {
  if (i == 2) {
    ...
    plot(febr_data["observacao_id"], add = TRUE, col = "black")
  } else {
    j <- ifelse(i < 2, i, i - 1)
    ...
    idx <- (idx == 2)
    plot(brazil, reset = FALSE, axes = FALSE, graticule = TRUE, main = main,
      col = "white", col_graticule = "white")
    plot(febr_data[idx, "observacao_id"], add = TRUE, col = "firebrick")
  }
}
dev.off()
